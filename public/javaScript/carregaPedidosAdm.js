async function carregaPedidosPorStatus(
  status,
  containerId,
  loadingId,
  filterDate = null
  ...
  if (response.ok) {
    ...
    let pedidosFiltrados = data.filter(
      (pedido) =>
        ...
        (status === "finalizado" && pedido.status === "cancelado")
    );
    if (filterDate) {
    ...
    pedidosFiltrados.forEach((pedido) => {
      ...
      pedidoItem.innerHTML = `
            ...
            <div class="card-body">
                <ul class="list-group">
                    <li class="list-group-item"><strong>Itens:</strong> ${pedido.itensPedido.replace(
                      /#/g,
                      "<br>"
                    )}</li>
                    <li class="list-group-item"><strong>Detalhes:</strong> ${
                      pedido.detalhes
                    }</li>
                    ...
                    <li class="list-group-item"><strong>Número:</strong> ${
                      pedido.numeroCelular || "0"
                    }</li>
                    <li class="list-group-item"><strong>Endereço:</strong> ${
                      pedido.endereco || "No local"
                    }</li>
                    <li class="list-group-item"><strong>Mesa:</strong> ${
                      pedido.mesa || "Entrega"
                    }</li>
                    <li class="list-group-item"><strong>Data:</strong> ${formatarData(
                      pedido.data
                    )}</li>
                    <li class="list-group-item"><strong>Forma de Pagamento:</strong> ${
                      pedido.formaPagamento
                    }</li>
                    <li class="list-group-item">
                        <strong>Taxas:</strong> <span class="badge bg-warning text-dark">R$ ${pedido.taxas.toFixed(
                          2
                        )}</span>
                    </li>
                    <li class="list-group-item">
                        <strong>Total:</strong> <span class="badge bg-success">R$ ${pedido.totalPedido.toFixed(
                          2
                        )}</span>
                    </li>
                </ul>
...
function confirmarAtualizacao(pkPedido, novoStatus) {
  if (
    confirm(
      `Tem certeza que deseja atualizar o pedido #${pkPedido} para "${novoStatus}"?`
    )
  ) {
  ...
  window.addEventListener("scroll", async () => {
    if (
      ...
      const currentPedidos =
        document.querySelectorAll("#rowPedFd .card").length;
